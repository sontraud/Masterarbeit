...
\texttt{OpenFOAM}, short for Open Source Field Operation and Manipulation, is a \lstinline|C++| based toolbox for continuum mechanics problems, including computational fluid dynamics.
Provided in \OF are pre-processing tools, e.g. for meshing, solvers, e.g. the \lstinline|MPPICFoam| and post-processing tools which we will use later in the results chapter. \todo{paraview kommt irgendwann später (oder so)}
...
\caption{Adapted from \cite{fileStruc}.}
\label{fig:fileStruc}
\end{figure}

In the \lstinline|0| directory we define the initial conditions, in this case for the fluid velocity and the kinematic pressure.
...
In the time directories the data of the specified fields in output during the simulation. In our case these are among other outputs the fluid velocity, fluid volume fraction, pressure and the \lstinline|lagrangian| folder which includes output about the velocity and position of the particles.

\section{\MP method in OpenFOAM}
Although the \MP method was presented in the mid 90s \cite{MPPICFirst} and extended to a three dimensional method in 2001 \cite{MPInterpolation}, it was only implemented in \OF in 2014 \cite{OFMPIntro}.
The solver in \OF is called \lstinline|MPPICFoam| and can be executed by running the likewise named command.
The solver is located under \lstinline|applications/solvers/lagrangian/| \lstinline|DPMFoam/MPPICFoam|. \todo[color= kit-blue!60]{Soll noch kurz was zu DPMFOAM?}
...
\begin{tikzpicture}[
    ...
    lane/.style={draw, rounded corners, fill=kit-gray!10, minimum width=42mm, minimum height=85mm},
...
The damping and packing models are prepared. The properties required for the models are cashed.
\ifcode
...
Because the results discussed in also do not show promising results we do not discuss the implementation here.
...
The gas phase time step is split into several smaller time steps in which articles are moved. At the beginning of \lstinline|move| the time fraction is set to zero.
...
For the Gaussian standard distribution a Box-Muller \todo[color=kit-blue!60]{Box-Mueller erklären?} sampling algorithm is used. 
Because
\begin{equation*}
...
\begin{equation*}
    \phi_{new} = \alpha \phi_{new} + (1- \alpha ) \phi_{old}
\end{equation*}
...
We start with the momentum equation which is build in \lstinline|UcEqn|.
...
\begin{align*}
    ...
    \color{kit-royalblue}-  \underbrace{\dfrac{S_u}{\rf}}_{\text{in } \texttt{cloudSU} \text{ and } \texttt{cloudVolSUSu}}
    \end{align*}
...
The equation is assembled in three steps with the first one shown above in listing \ref{code:Uc1} assembling. The equation is relaxed to ensure diagonal-dominance needed for convergence. This is discretized as
\begin{align*}
...
These are added in the next step.
\begin{lstlisting}[caption = UcEqn.C]
solve
    (
        UcEqn
     ==
