...
Provided in \OF are pre-processing tools, e.g. for meshing, solvers, e.g. the \lstinline|MPPICFoam| and post-processing tools which we will use later in the results chapter. \todo{paraview würde ich erst später erwähnen}
...
The \MP method in \OF is called \lstinline|MPPICFoam| and can be executed by running the likewise named command.
The solver is located under \lstinline|applications/solvers/lagrangian/| \lstinline|DPMFoam/MPPICFoam|. The \MP method is an incompressible transient solver that calls upon the \lstinline|Lagrangian| framework to solve for the particle trajectories. 
...
The implementation of the \MP method in \OF is partially explained in \cite{reportMPPIC}, \cite{chimera} and \cite{ImplemInter}. The former focuses more on the current \MP implementation, while the latter extends the current implementation to include a chimera mesh or respectively coarse particles.
...
Afterward, the particles are inserted as a kinematic cloud. The upper limit of the particle volume fraction is calculated by using the maximum packing, which can be defined in \lstinline|kinematicCloudProperties|.
...
\centering
    ...
    \tikzset{stretch/.initial=.5}
        ...
        \node (tau) [decision, below of = mv1] {$\tvar_{lag} < \tvar_{n+1}$};
...
\end{figure}
\FloatBarrier
In this section we are going to exclude the collision damping model described in \ref{sec:DiscCollDamp} as stated in \cite{chimera} including this model currently causes errors.
Because the results discussed in \Todo{Referenz} also do not show promising results we do not discuss the implementation here.
...
\end{equation*}
Since small changes in the particle solid stress can have a big influence the particle normal stress the velocity is updated in a first step without the particle normal stress. 
...
In the \OF implementation the drag term is neglected due to it being either dominated by the pressure term or insignificant. Instead a small factor is included
\todo{stimmt das?}
\begin{equation*}
...
% \end{equation*}
as well as the Lun particle normal stress model. 
...
This implies that the probability of collision goes towards $1$ for the damping term approaching zero or in physical terms that when we expect the time until the next collision to be zero that there is a collision, see figure 
...
For the Gaussian standard distribution a Box-Muller sampling algorithm is used. 
Since 
\todo{Zählt das noch als Allgemein\-bildung oder zitieren?}
\begin{equation*}
    X \sim \mathcal{N}(\mu , \sigma^2) \qquad \Rightarrow \qquad aX+b \sim \mathcal{N} (a \mu + b, a^2 \sigma^2)
\end{equation*}
we can avoid calculating the Gaussian distribution but instead shift the normal distribution.
...
The weights are proportional to the sub-tetrahedrons that are formed using $\pbar$, see \ref{fig:tetSub}.
%\Todo{Draw a picture}
...
\todo{Könnte Beispiel einfügen $\lambda_a = 0$ -> $p \in (b,c,d)_\triangle$ }
...
\todo{Zshg Interpolation Stepfraction erläutern?}
...
The momentum transfer is included in \lstinline|cloudSU|. This includes two terms, one for
Afterward the momentum transfer term is set. 

\Todo{Cite Forum} %https://www.cfd-online.com/Forums/openfoam-post-processing/137291-questions-about-lagrangian-postprocessing.html
...
The implicit part \lstinline|cloudSU| is added in \lstinline|UcEqn| and the explicit part  in \lstinline|pEqn|. \todo{Das Paper lügt vermutlich, kommt beides in UcEqn}


We start with the momentum equation which is build in \lstinline|UcEqn|.
\begin{lstlisting}[caption=\texttt{UcEqn.C}]
...
\begin{align*}
    ...
    - \dfrac{f_d}{\rf}
\end{align*}
with the third term, which is zero due to the continuity equation, being added for numerical stability.
Here we divided by the fluid density $\rf$. From here on we therefor solve for the kinematic pressure.
...
\begin{align*}
    A_P \ufc + \sum_N A_N \ufn - S_p = - (\fvf \nabla \p)_P - g - \left. \dfrac{f_p}{\rf}  \middle|_c \right.
\end{align*}
...
For an arbitrary cell we therefor solve the equation
\begin{equation*}
    \up =  \dfrac{H(u)}{a_p} - \dfrac{\nabla \p}{a_p}.
\end{equation*}
Here we have three equations for four variables, three for the velocity and the pressure. 
The 

For the cell faces we then have
\begin{equation*}
    \ufc = \left( \dfrac{H(u)}{a_p} \right)_f - \left( \dfrac{\nabla \p}{a_p} \right)_f.
\end{equation*}
We combine this with the continuity equation
\begin{equation*}
    \sum_f S \cdot \fvf \left( \dfrac{\nabla \p}{a_p}\right)_f = \dfrac{\partial \fvf}{\partial \tvar} + \sum_f S \cdot\fvf \left(\dfrac{H(u)}{a_p}\right),
\end{equation*}
where we used the Gauss' theorem to get the mass face flux $S \cdot (\fvf \u)_f$.

\ifkeepcode
\begin{lstlisting}[caption=\texttt{pEqn.C},showlines=true]
    fvScalarMatrix pEqn
        (
...
initial guess p alter Zeitwert?, wieso verwenden wir ständig flux und nicht velocity at face, Non orthogonal sind wir nicht aber brauchen den Teil für die Iteration?}
\todo[inline]{Was wurde hier noch nicht behandelt was aber evtl. relevant wäre: Wie funktioniert die Gruppierung von Particles zu Parcels?}
