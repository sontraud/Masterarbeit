...
\centering
  ...
  ($ ({tau.east}|-{tau.east}) + (6,0) $) -- node[anchor=east] {$\tvar_{lag,n+1} = \tvar_{lag,n} + (\tvar_{n+1}-\tvar_n) \tvar_{frac}$}
...
All three are explained in the Notes on Computational Fluid Dynamics \cite{cdfNotes}.
...
Here we divided by the fluid density $\rf$. 
...
This concludes the current time step of the \MP method. At the beginning of the next time step, the simulation time is compared to the user-defined end time. If the stop criterion is met, the iteration is terminated. Otherwise, we start again by updating the viscosity with Eulerian field variables.
