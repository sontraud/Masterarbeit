...
\end{figure}
\todo[color=kit-blue!60]{In dem Bild säh ein zweiter und dritter Tetraeder schon noch schön aus}
%\Todo{Bild einfügen}
...
\todo[color= kit-blue!60]{Inter\-polation bei bewegen durch mehrere Zellen erläutern?}
...
If particles move through serveral tetrahedron during the \lstinline|motion| they are moved to the face. The particle is then tracked in the next tetrahedron.
...
\begin{equation*}
    S_u = \dfrac{\u_{trans}}{V \Delta \tvar} - \dfrac{\u_{coeff}}{V \Delta \tvar} \up^{n+1} + \dfrac{\u_{coeff}}{V \Delta \tvar} \up^{n}.
\end{equation*}
%\todo{Wie zitiere ich ein Bild aus einem Forum? (\href{https://www.cfd-online.com/Forums/openfoam-post-processing/137291-questions-about-lagrangian-postprocessing.html}{Link}) Will ich das überhaupt?} 
The first term is the momentum transfer while the other two include drag, gravity and pressure gradient forces once implicitly and once explicitly.
...
\begin{align*}
    \dfrac{\partial}{\partial \tvar} (\fvf  \uf ) &+ \nabla \cdot (\fvf \uf \uf) - \uf \left( \dfrac{\partial \fvf}{\partial \tvar} + \nabla \cdot (\fvf \uf) \right) - \nabla \cdot (\fvf \ss) = 
    -g +  \nabla \dfrac{\p}{\rf} 
    - \dfrac{f_d}{\rf}
    \end{align*}
\todo[inline, color= kit-blue!60]{Je nachdem welchen Ansatz man wählt müsste hier $\fvf\nabla \cdot \tau_c$ oder $\nabla \cdot (\fvf \tau_c)$ stehen, Orginal Paper wählt $\tau_c = 0$ da macht das keinen Unterschied aber hier schon...}
...
The equation is assembled in three steps with the first one shown above \ref{code:Uc1} assembling. The equation is relaxed to ensure diagonal-dominance needed for convergence. This is discretized as
\begin{align*}
    A_P \ufc + \sum_N A_N \ufn - S_p = - \dfrac{f_p}{\rf} 
\end{align*}
...
Here \lstinline|phicForces| contains the explicit volume momentum source term and the gravity force. 
They are explicitly discretized and added to the right hand side of our equation.
Solving our discretized equation for an arbitrary cell we have
\begin{equation*}
    \u_{c,P} =  \dfrac{H(u)}{a_p} - \fvf \dfrac{\nabla \p}{a_p} - \dfrac{g}{a_p} -  \dfrac{f_p}{\rf a_p}  -\dfrac{1}{a_p} S_p  \eqqcolon \dfrac{\tilde{H}(u)}{a_p} - \dfrac{\nabla \p}{a_p}
\end{equation*}
...
The poisson pressure equation is solved
%For the cell faces the equation 
\begin{equation*}
    \ufc =  \dfrac{\tilde{H}(u)}{a_p} -\dfrac{\nabla \p}{a_p} .
\end{equation*}
is solved.
We combine this with the continuity equation
\begin{equation*}
...
initial guess p alter Zeitwert?, wieso verwenden wir ständig flux und nicht velocity at face, Non orthogonal sind wir nicht aber brauchen den Teil für die Iteration?, Aufteilung des }
\todo[inline]{Was wurde hier noch nicht behandelt was man aber behandelen könnte: Wie funktioniert die Gruppierung von Particles zu Parcels?, Wie funktioniert Wandreflektion? Impliziter solid stress term?, Drag Modelle aber das kommt beim Setup (nächstes Kaptiel)}
