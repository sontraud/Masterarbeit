...
In this case the minimal relative error for the pressure drop in figure \ref{fig:C1PDEWYStoch0015} of $0.23$ is at $\beta = 2.0$, therefor at the other end of the spectrum. The close pack volume fraction at the minimal relative error is at $\pvf = 0.65$.
The pressure drop peaks at $\beta = 3.0$ and $\pvfcp = 9.55$ with a relative error of $0.30$. 
The bed height in figure \ref{fig:C1BHEWYStoch0015} is similar to the previous case. Again we see a plateau with a steep slope descending towards high particle volume fractions and exponents.
...
The relative errors for the pressure drop, figure \ref{fig:C1PDPMNone0015} and the bed height, figure \ref{fig:C1BHPMNone0015} show similar trends, both descending for a higher exponent and bigger particle volume fractions.
...
The overall relative error, figure \ref{fig:C1PMNone0015}, shows the same trend with the minimal relative error at $\pvfcp = 0.68$ and $\beta = 5.0$ of $0.29$ and a maximal relative error at $\pvfcp = 0.6$ and $\beta = 3.0$ of $0.36$.
...
Including the isotropy model in our simulation again switches the impact of the exponent on the pressure drop in figure \ref{fig:C1PDPMStoch0015}.
...
The bed height again plateaus with a maximal error of about $0.22$ at $\beta = 2.5$ and $\pvfcp = 0.7$ in figure \ref{fig:C1BHPMStoch0015}. \todo{Wo Betthöhe minimum}
...
The bed height dominates the overall relative error in figure \ref{fig:C1PMStoch0015} for large exponents and large close pack particle volume fractions and leads to a decline. The error plateaus where both errors are big, therefor at intermediate $\beta$ and declines for smaller exponents analogously to the relative error of the pressure drop.
...
% \subsection{Case 1: Variation of αcp​ and β}


% With Isotropy Model (\lstinline|stochastic|) Activating the stochastic isotropy model inverts the dependency of the pressure drop on β. The minimum pressure error (0.23) shifts to the lower end of the spectrum (β=2.0,αcp​=0.65). However, the bed height error remains consistent with the previous case, favoring high exponents (minimum error 0.04 at β=5.0). Since the bed height error gradient is steep, it influences the combined error significantly. Despite the shift in pressure drop behavior, the global optimum remains at high β values, though the error landscape is flatter compared to the non-isotropy case.

% \FloatBarrier \begin{figure}[h!] \centering \begin{subfigure}[b]{0.49 \linewidth} \includegraphics[width = \linewidth, trim = {3.5cm 0 3.5cm 0}, clip]{sections/result/Case1Kin/kin_Z_pres0.015_ErgunWenYuDrag_stochastic.png} \caption{Pressure drop error.} \end{subfigure} \hfill \begin{subfigure}[b]{0.49 \linewidth} \includegraphics[width = \linewidth, trim = {3.5cm 0 3.5cm 0}, clip]{sections/result/Case1Kin/kin_Z_height0.015_ErgunWenYuDrag_stochastic.png} \caption{Bed height error.} \end{subfigure} \caption{Relative error maps for \lstinline|ErgunWenYuDrag| with \lstinline|stochastic| isotropy.} \end{figure}
...
The high relative pressure drop error could be due to the different locations where we measure the pressure in the simulation vs the experiments.
In the following figure \ref{fig:expPressMeasurePlace} is a sketch of the experimental setup.
\begin{figure}
    \centering
    \includegraphics[width=0.33\linewidth, trim = {2cm 10cm 6cm 2cm},clip]{sections/result/Reaktorplanung.png}
    \caption{Setup of the experiments used to collect reference data.}
    \label{fig:expPressMeasurePlace}
\end{figure}
The reference data pressure at the bottom was collected beneath the mesh.
...
In the simulation we therefor revert to evaluating the bottom pressure right on top of the inlet, which would be the mesh in the experiments.
...
The model managed to capture the bed height more accurately, though it remains unclear if the bed height gathered from the experiments is accurate, especially at higher inlet velocities due to dust settling on the tube and being nearly indistinguishable from the bed. The image wobbles from time to time further increasing the error.
