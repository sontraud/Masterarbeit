...
Using iron as a carbon-free energy carrier both reaction steps, the oxidation and the reduction need to be as efficient as possible.
Currently, further studies into which reactor type give the most promising results for the iron oxide reduction are needed \cite{StudyForReactorNecessary}.
...
%     \label{fig:malteFBR}
% \end{figure}
In a fluidized bed reactor solid particles are in suspension by a upward flowing fluid, in our case a reducing gas.
This induces fluid-like properties in the particles, like the ability to maintain a horizontal surface when the reactor is tilted \cite{zustand_eng}.
...
On the left, the gas inflow rate is low. The gas percolates through the empty spaces in-between the particles. The particles form a fixed bed and the particles are still stationary.
As the flow rate increases, the bed begins to expand.
In the second illustration, the inflow velocity is increased until minimum fluidization $\u_{mf}$, where the upward drag force equals the gravitational force on the bed.
...
For an even higher velocity we reach the terminal velocities at which for a single particle forces acting on the particle, e.g. drag are balanced. Exceeding this velocity can lead to particles leaving the reactor.
\begin{figure}[h!]
    \centering
    ...
    \caption{Fluidization state in a fluidized bed reactor for different inflow velocities, from left low velocity to right high velocity \cite{zustand_eng}.}
    \label{fig:fluidize_states}
\end{figure}
...
To characterize the different states experimentally we can look at the pressure drop in fig \ref{fig:presDropRegime}.
Up to the minimum fluidization velocity the pressure drop grows practically linearly. 
At the transition from fixed bed to fluidized bed the pressure drop can grow slightly until the adhesive forces of the particles are overcome.
...
We can see that the bubbling fluidized beds are achieved for velocities above the minimum fluidization velocity. For larger particles the gap in dimensionless velocity between the two decreases.
The terminal velocity for smaller particles co-occurs with the bubbling fluidized bed. 
...
The fluid is generally treated as a continuum, therefore its properties, such as volume fraction, pressure, velocity, are modeled by continuous fields. 
The dispersed phase as shown in fig \ref{fig:solidClass} can be modeled by one of two main approaches.
\FloatBarrier
\begin{figure}[h!]
\centering
    \begin{tikzpicture}[
...
In \OF simulations are run with the \lstinline|twoPhaseEulerFoam| solver.
Although this model is computationally less expensive the modeling used requires closure relations. Generally more closure relations lead to less model accuracy but also less computation time \cite{mathMPPIC}.
...
To mitigate the disadvantages from both methods, we use a hybrid model, called the Multiphase-Particle-In-Cell (\MP\hspace{-12pt}) method.
...
The continuous properties of both phases are then calculated via on the continuous. The continuous properties are interpolated back to the particles and used to calculate the following advanced time step.
the acceleration of the particles due to collisions is calculated as done in the Euler-Euler method by a solid stress tensor.
