...
\chapter{Background}
\label{ch:background}
This chapter introduces the principles of a fluidized bed reactor. It examines different operation velocities and the resulting bed behaviors. Furthermore, the fundamentals of modeling gas-solid systems are discussed, presenting the \MP method as a hybrid modeling approach
Furthermore, the fundamentals of modelling gas solid systems is discussed with the \MP method being introduced as a hybrid model.
...
Particles are either inserted into the reactor via a chute or placed inside the reactor on top of a porous plate. The accumulated particles are called a bed.
...
For even higher velocities, we reach the terminal velocity at which, for a single particle, forces acting on the particle, buoyancy, drag and gravitational force, are balanced. Exceeding this velocity, the particle starts to move with the gas, which can lead to particles leaving the reactor.
\begin{figure}[h!]
    \centering
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{sections/background/zustand_englisch.png}
    \caption{Fluidization regime diagram for the dimensionless particle parameter and inlet velocity \cite{zustand_eng}.}
    \label{fig:fluidizeDiagram}
\end{figure}
