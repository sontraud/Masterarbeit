...
We can classify CFD simulations by how the solid phase is modeled, whether we model it as a continuum or a discrete phase, while the gas phase is modeled as a continuum. 
The models differ in how particle-particle interactions are handled.
Using a solver with a discrete particle phase, such as the \DP method in \OF we can model each particle-particle interaction, therefor each collision and the resulting behavior of the particles. With an increasing number of particles, the simulation time grows quadratically with the number of particles. 
For industrial scale applications it is often more important to look at the trend of an experiment. For these cases one could consider modeling the particle as a continuum would result in an Euler-Euler approach where particle-particle interactions are not considered.
This leads to a computationally inexpensive method that does not account for particle-particle interactions. 
To midigate the disadvantages from the previously mentioned methods we will use a hybrid approach which models particle interactions through the particle normal stress and damping terms without resolving them, called the \MP method.
In this method the particles are modeled as both a discrete and a continous phase with interpolation operators being used to transfer properties between the two.
