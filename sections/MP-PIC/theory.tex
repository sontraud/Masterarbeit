...
\begin{align}
    ...
    \up^{n+1})  - \dfrac{1}{\rp \pvf} \nabla_{\xp} \scs^{n+1} + \g \\
    \label{eq:MPvelocity}
    \Leftrightarrow \up^{n+1} &= \dfrac{\up^n + \Delta \tvar \left[ \d \ip \uf^{n+1} - \dfrac{1}{\rp} \nabla_{\xp} \ppi^{n+1} - \dfrac{1}{\rp \pvf} \nabla_{\xp} \scs^{n+1} + \g \right]}{1 + \Delta \tvar \d} .
\end{align}
The former two equations \eqref{eq:MPposition}, \eqref{eq:MPvelocity} are used at the end of a time step to calculate the new particle velocities and positions.
...
\begin{align*}
    \sum_i \sum_p \left( \dfrac{\Np \mp}{\rp} \ifg_{i+\frac{1}{2}} ( \xpp)\right)\dfrac{\tilde{\ss}_{i+1}- \tilde{\ss}_i}{\frac{1}{\Delta \x}\sum_p \dfrac{\Np \mp}{\rp} \ifg_{i+\frac{1}{2}} ( \xpp) \Delta \x} = \sum_{i=0}^{N} \tilde{\ss}_{i+1} - \tilde{\ss}_i = \tilde{\ss}_N - \tilde{\ss}_0.
\end{align*}
The last to term are simplified to the one dimensional case but can be extended to two or three dimensional cases.
The remaining terms are boundary sources thus proving momentum conservation.
...
Once the grid values are calculated we can approximate the new particle positions by equation \eqref{eq:MPvelocity} and use the value to calculate the new particle positions. 
