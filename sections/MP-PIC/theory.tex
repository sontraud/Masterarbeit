...
In this chapter, we will look at how the \MP method works. First we will look at which governing equations are used, especially how the particles are modeled.
 Afterward we will discuss the first numerical implementation based on the paper which first presented the \MP method, \cite{MPPICFirst}.
...
\begin{align*}
    ...
    \Leftrightarrow
    \dfrac{\partial }{\partial \tvar} \int \int \pd \m d \m d \up + \nabla \cdot \left( \int \int m \pd \up d\up d \m \right) = 0\\
...
We first solve the equation without the collision
\begin{equation*}
...
The second damping term, called return-to-isotropy, relaxes the velocities to a Gaussian velocity distribution. The equilibrium distribution is a particle distribution function with a Gaussian velocity distribution
\begin{equation*}
...
\end{equation*}
To determine whether a particle collides, we use the return-to-isotropy time, which is similar to a relaxation time
\begin{equation}
...
\end{equation}
that approximates the time until the next collision. Here we have the particle Sauter mean radius
\begin{equation*}
...
\begin{tikzpicture}[ultra thick, scale = 1.4]
    ...
    \draw[kit-yellow] (0,0) -- (2,0) -- (3,1) -- (4,0);
    \node[anchor = north] at (1,-0.25) {$x_{i-1}$};
    \node[anchor = north] at (2,-0.25) {$x_{i}$};
    \node[anchor = north] at (3,-0.25) {$x_{i+1}$};
    %\draw[dotted] (1.5, 0.25) -- (2.5,0.25) -- (2.5, -0.25) -- (1.5, -0.25) -- cycle;
...
In \texttt{OpenFOAM} cells are divided into tetrahedrons
\Todo{Tetrahedon}
...
For the initial discrete distribution $\pd_0$ we can use the velocity and positions from the previous time step
\begin{equation*}
    \pd_0 = \sum_p \Np \delta ( \x - \x_p^n) \delta ( \u - \up^n) \delta (\m - \mp).
\end{equation*}
...
\end{equation*}
The advanced particle distribution is obtained by randomly sampling. The details are explained in the implementation.
If we concur that no collision has taken place the velocity of the particle remains the same as before. Else we draw from the Gaussian distribution and add that velocity to the current particle velocity.
...
To conserve the mean particle velocity and variance cell-wise we modify the particle velocity $\tilde{\up}$ that includes collision. The new velocity is 
\begin{equation}
\label{eq:rtiUpdate}
...
\begin{align*}
\sum_p \Np \mp \up^{n+1} &= \sum_p \overline{\u} + \alpha \left( \sum_p \Np \mp \tilde{\up} - \sum_p \Np \mp \tilde{\up}\right) = \sum_p \Np \mp \overline{\u} \\
\dfrac{\sum_p \Np \mp \left( \up^{n+1} - \overline{\u} \right)}{\sigma^2} &= \sum_p \Np \Np \dfrac{\sum_p \Np \mp}{\sum_p \Np \mp ( \tilde{\up} + \delta \u - \u)^2} (\tilde{\up} + \delta \u - \u)^2 \\
&\Rightarrow \sum_p \Np \mp \left( \up^{n+1} - \u \right)^2 = \sigma^2 \sum_p \Np \mp.
\end{align*}
