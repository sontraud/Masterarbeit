...
The stress tensor models particle-particle interactions and, in addition to further Eulerian properties, we use them to advance the particle positions. \todo{wird der stress tensor für translation verwendet?}
...
The number density of particles at position $\x \in \Omega$, with velocity $\u \in \Pi$, mass $\m$ at time $\tvar$ is given by the particle probability distribution function \todo{hier geht es doch um eine position x aus der domain oder? oder doch ein teilvolumen, dann solltest du es zurück ändern und den rest anders schreiben}
\begin{equation}
...
The probable number of particles in the given volume the number of particles with a velocity in and a mass of $\m$ at time $\tvar$ is not necessarily contained since fluctuations can occur but is the average number of particles when the time $dt$ is smoothed out. \todo{ganzen satz überdenken, ich kann leider nicht ganz raten was ich hier korrigieren soll}
...
\end{align}
%Op particle volume
Here $\uf$ is the flow fluid velocity, $\d(d_p,\up, \xp,\tvar)$ is the drag function, discussed in Sec. \ref{sec:MPDrag}, $\scs(\xp, \tvar)$ is the solids contact stress, discussed in Eq. \ref{eq:solidStress} and $\p$ is the mean flow fluid thermodynamic pressure and $d_p$ is the particle diameter. \todo{flow fluid oder fluid flow? (2x in diesem satz)}
...
\end{equation}
By replacing the corresponding terms in equation \eqref{eq:mom2Begin} and multiplying by $\pvf \rp$ we then obtain the particle momentum equation
\begin{equation}
    \begin{aligned}
\label{eq:secondMomentum}
    ...
    \end{aligned}
\end{equation}
The last term on the right-hand side in the equation mimics the kinematic stress from the particle velocity fluctuations, similar to Reynolds stress. The last term on both the left and right hand side of the equation depends on both phases and therefore contributes to the interphase momentum transfer function, which models the particle force per unit volume on the fluid phase
...
\end{equation}
which is included in equation \eqref{eq:InterMomTrans} as part of the interface momentum transfer function. 
By equating both terms with one another, only $\pvf \nabla p$ remains in equation \eqref{eq:InterMomTrans}.
...
The gradient of the particle stress $\scs$ is included in the governing equations of the particles, specifically in the acceleration of the particles Eq. \ref{eq:accel}, and it accelerates particles away from close-packed regions to prevent over-packing. \todo{überprüfen ob der inhalt in dem satz so passt}
...
For example, we can consider a particle that is moving away from a packed region, e.g. a packed bed. Due to the change in solid stress, the particle would be accelerated further away from the packed bed, which is physically unrealistic.
...
This physically represents a model where, for the first term no collision has taken place at time $[0,t]$ which happens with a probability of $\operatorname{exp}(-\tvar / \ss_G)$, while a collision has taken place with the probability of $1 - \operatorname{exp}(-\tvar / \ss_G)$. \todo{ist model hier das richtige wort?}
...
The provisional particle position is used to calculate a provisional particle fraction. 
...
\end{tikzpicture}
\caption{Grouping particles on the left to parcels on the right with $\wp = 3$. On the right particles are shown as "point masses" as used in the Lagrangian framework. The two sketches on the left are for illustraive purposes to show how grouping works and do not represent the particles used for calculation.}
\end{figure}
\FloatBarrier
...
\end{figure}
\todo{figure caption 2x on the right, ich bin aber nicht 100\% sicher was du mit der caption sagen willst, vllt bissl ausführlicher}
...
Furthermore can also map the gradient of a grid property $\gp$ to and from the particle \todo{den satz verstehe ich nicht}
\begin{equation*}
...
Then, the momentum and continuity equations are solved on the Eulerian map. \todo{eulerian grid or map?}
