...
\end{equation}
see \cite{mathMPPIC}.
We also assume that we have an ideal gas, which we will use later in the numerical methods. Therefor the pressure can be related to the fluid density by
\begin{equation}
...
$\pd : \Omega \times \Pi \times I \times \R \to \R^+, \pd (\xp , \up , \tvar, \m)$
...
\end{equation*}
By replacing the corresponding terms in equation \eqref{eq:mom2Begin} and multiplying by $\pvf \rp$ we then have
\begin{equation}
    \begin{aligned}
...
A second term  was introduced later (\cite{damp}, \cite{isotropy})  which represents the scattering from particle collisions and is called return-to-isotropy. The terms will be explained in the following.
\begin{equation}
...
\centering
    ...
    \tikzset{stretch/.initial=1}
        ...
        \node (pro1) [process, below of=in1] {Tridiagonal solver for $\pvf$ and $\fvf \rf$};
        ...
        \node (dec1) [process, below of=pro2] {Calculate coefficients};
        \node (stop) [startstop, below of = dec1] {End time step loop};
