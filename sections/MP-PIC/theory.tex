...
\end{equation*}
We can calculate the gradient of the linear interpolation operator with the help of
\begin{equation*}
...
\begin{figure}[h!]
    ...
    %\node[anchor = south] at (2, 0.25) {cell $\# i$};
\end{tikzpicture}
\caption{Interpolation operator $\ip_i$ in purple and the its gradient in blue. $\Delta x$ is one.}
\end{figure}
...
For the pressure we use the assumption from above \eqref{eq:gasDensReal} that fluid pressure and the density are related and receive
\begin{equation}
...
For the \MP-method a total of four equations have to be solved. In the following we will discuss where these equations originate from.
