...
\label{ch:MPPIC}
In this chapter, we discuss the theoretical background of the \MP (Multi‑Phase Particle‑In‑Cell) method. We look at the governing equations and how they are solved and at the modeling approaches, especially the methods used for the particles.
...
In the \MP method, the fluid phase is described using an Eulerian, therefore continuous, framework, whereas the particle phase is represented using a Lagrangian, therefore discrete, approach. 
For the fluid phase, mass and momentum equations are solved. 
For a reactive solver we would also need to solve species and energy equations.
...
For the particle behavior we look at the first and second moments of the Liouville equation.
Obtained particle properties are projected onto an Eulerian grid, where they are used to compute the solid stress tensor.
...
In this section we will look at the governing equations for both phases, as well as the interpolation operators that couple them and the numerical solution.
...
we have $t$ as the time, $\p$ as the fluid pressure and $\fst$ as the fluid stress tensor. $\F$ is the interphase momentum transfer, which will be discussed in the following subsection. 
For both equations we assume that we have no interphase mass transfer.
The fluid stress tensor was omitted from the equations in the original series of papers that presented the method \cite{MPPICFirst}, \cite{MP2d},\cite{MPInterpolation}, therefore it was set to zero
\begin{equation}
\label{eq:FlStTeZero}
    \fst = 0,
\end{equation}
...
The solid phase is treated as both a continuous phase and a discrete phase in the \MP method.
...
The number density of particles at position $\x \in \Omega$, with velocity $\u \in \Pi$, mass $\m$ at time $\tvar$ is given by the particle probability distribution function
\begin{equation}
...
The particle density function gives the probable average number of particles per unit volume $\Np$ with velocity in the interval $(\Vp, \Vp + \Delta \Vp)$ and mass in the interval $(\M, \M + \Delta \M)$ when integrating over the velocity and mass
...
Therefore, the above predicted number is not necessarily contained since fluctuations can occur but is the average number of particles when the time $dt$ is smoothed out.
In the following, we will assume that $\pd$ is non-negative, finite, continuous and goes to zero as the velocity becomes infinite.

The solids volume fraction is then given by integrating over particles of all masses and velocities
\begin{equation}
    \label{eq:svf}
...
We assume that the total number of particles near $\xp$ with velocity $\up$ in any volume is conserved. Therefore, we have
\begin{equation}
...
Using that this holds for all volumes, we can transform the equation above into what the authors refer to as the Liouville equation
...
\end{align}
%Op particle volume
Here $\uf$ is the fluid flow velocity, $\d(d_p,\up, \xp,\tvar)$ is the drag function, discussed in Sec. \ref{sec:MPDrag}, $\scs(\xp, \tvar)$ is the solids contact stress, discussed in Eq. \ref{eq:solidStress} and $\p$ is the mean fluid flow thermodynamic pressure and $d_p$ is the particle diameter.
We account for the effects of collisions by the spatial gradient of the solid stress. 
Adding collision terms on the right-hand side of the Liouville equation \eqref{eq:Liouville} we can model the behavior of particles after a collision more locally
\begin{equation}
...
\end{aligned}    
\end{equation}
with the third term, the acceleration term, being zero by using integration by parts and because $\pd \to 0$ for $\lvert \up \rvert \to \infty$.
...
\end{equation}
By replacing the corresponding terms in Eq. \eqref{eq:mom2Begin} and multiplying by $\pvf \rp$ we then obtain the particle momentum equation
\begin{equation}
    \begin{aligned}
\label{eq:secondMomentum}
    ...
    \end{aligned}
\end{equation}
The last term on the right-hand side in the equation mimics the kinematic stress from the particle velocity fluctuations, similar to Reynolds stress. The last term on both the left- and right-hand side of the equation depend on both phases and therefore contribute to the interphase momentum transfer function, which models the particle force per unit volume on the fluid phase
%\Todo{Second momentum term}
\begin{equation}
    \label{eq:InterMomTrans}
    \F = \MultiIntegral{2} \pd \m [\d (\uf - \up) - \dfrac{1}{\rp} \nabla \p] d\m d\up.
\end{equation}
We can split the integral into two terms. The first represents the momentum exchange due to aerodynamic drag.
%For more detailed models we can use particle density and volume instead of mass.
In the gas momentum Eq. \eqref{eq:momEul}, the pressure gradient is multiplied by the gas volume fraction. This term arises from the second term in \eqref{eq:InterMomTrans}, as shown by the following calculation.
We know that for the volume fraction
\begin{equation}
    \fvf + \pvf = 1 \qquad \nabla p - \fvf \nabla p = \pvf \nabla p
\end{equation}
holds.
From Eq. \eqref{eq:svf} we have
\begin{equation}
    \fvf \nabla p =  \int \int \pd \m \dfrac{1}{\rp} \nabla \p d\m d\up
\end{equation}
which is included in Eq. \eqref{eq:InterMomTrans} as part of the interface momentum transfer function. 
By equating both terms with one another, only $\pvf \nabla p$ remains in Eq. \eqref{eq:InterMomTrans}.
...
In the paper that first presented the method \cite{MPPICFirst}, the interphase drag model
\begin{align*}
    D_p = C_D\dfrac{3}{8} \dfrac{\rf}{\rp} \dfrac{\lvert \uf - \up \rvert}{r_p}
\end{align*}
was used with the drag constant
\begin{align*}
...
For high porosity values above $\phi \geq 0.8$, the model was extended by Wen-Yu \cite{WenYu}
\begin{align*}
    C_d =
    \begin{cases}
...
\subsection{Particle-Particle Interaction Modeling}
To model the behavior of particles in a dense region, we use an inter-particle stress.
The gradient of the particle stress $\scs$ is included in the governing equations of the particles, specifically in the acceleration of the particles Eq. \ref{eq:accel}, and accelerates particles away from close-packed regions to prevent over-packing.
Here, in Sec. \ref{sec:particleNormalStress}, we discuss two particle normal stress models: the Harris and Crighton model and the Lun model.
Submodels that resolve particle-particle interactions further are the collision damping term in Sec. \ref{sec:damping}, which relaxes the particle velocities towards the mass-averaged velocity
and the return-to-isotropy term in Sec. \ref{sec:isotropy}, which relaxes the velocity distribution towards a Gaussian velocity distribution.


\subsubsection{Particle Normal Stress Model}
\label{sec:particleNormalStress}
In the \MP method, the collisions between particles are approximated by an isotropic inter-particle stress. 
...
Particles approaching close packing, or in a closely packed cell, are accelerated to prevent over-packing.
...
For example, we can consider a particle that is moving away from a packed region, e.g. a packed bed. Due to the change in solid stress, the particle would be further accelerated away from the packed bed, which is physically unrealistic.
...
A more complex model that incorporates these effects was proposed by Lun \cite{Lun}.
...
\end{equation*}
In the parameter study in Ch. \ref{ch:results} we only use the Harris and Crighton model.

Superseding the paper introducing the one-dimensional approach, the \MP method was extended to two dimensions \cite{MP2d} and later to three dimensions \cite{MPInterpolation}.
Collision terms were introduced on the right-hand side of the Liouville equation. The first one is called collision damping \cite{damp} and damps the particle velocity by relaxing it toward the mass-averaged particle velocity, while the second term is called return-to-isotropy \cite{isotropy}, which represents the scattering from collisions and relaxes the particle velocities toward a Gaussian distribution.
These terms improve upon the \MP method by modeling particle collisions locally as opposed to using spatial gradients of the particle gradient.
\begin{equation}
...
\label{sec:damping}
The first term is known as the collision damping term. 
Two versions of this damping term exist.
The first is the equilibrium damping term, which we will not focus on here \cite{dampUnimproved}.
The second is the non-equilibrium term, which we will look at in the following \cite{damp}.
The two terms differ by the collision damping time.
The damping term is defined similar to a Bhatnagar,
...
\end{equation}
can be physically interpreted as the average time between collisions.
...
To apply the damping term, we first solve it considering only collision effects
\begin{equation*}
...
\end{equation}
which we can quickly verify by partially differentiating the equation above
\begin{equation}
    \dfrac{\partial \pd}{\partial \tvar} = - \dfrac{1}{\sd} e^{- \frac{t}{\sd}} \pd_n + \dfrac{1}{\sd} e^{- \frac{t}{\sd}} \pd_D = \dfrac{\pd_D - \pd}{\sd}.
\end{equation}

The implementation of the collision damping term is discussed in Sec. \ref{sec:DiscCollDamp}.


\subsubsection{Return-to-Isotropy}
\label{sec:isotropy}
The second damping term, called return-to-isotropy, relaxes the velocities to an equilibrium velocity distribution. The equilibrium distribution is a particle distribution function with a Gaussian velocity distribution
\begin{equation*}
...
% \end{equation*}
We assume that both PDFs $\pd$ and $\pd_F$ have the same variance. The variance of $\pd_F$ is given by $\sigma^2(m)$, which is then equal to the normalized mass-weighted variance
\begin{equation*}
...
\end{equation*}
To determine whether a particle undergoes a collision, we use the return-to-isotropy relaxation time
\begin{equation}
...
which approximates the average time until the next collision. The Sauter mean radius $r_{32}$ and the radial distribution function $g_0$ are defined as above.
The return-to-isotropy term is derived in the Appendix of \cite{isotropy} and is obtained by comparing the particle momentum shear viscosity to the shear viscosity of continuum-fluid models.
...
Physically, the first term represents the case where no collision has taken place at time $[0,t]$ which happens with a probability of $\operatorname{exp}(-\tvar / \ss_G)$, while a collision has taken place with the probability of $1 - \operatorname{exp}(-\tvar / \ss_G)$.
...
These provisional particle positions used to calculate a provisional particle volume fraction. 
...
For the continuous phase, we discretize the mass and momentum conservation and combine these with the discretized first and second moments from the Liouville equation and use the acceleration equation to update the particle positions. 
...
The sketch, fig. \ref{fig:Parcels} below illustrates the grouping of particles. The sketches on the left and in the middle are for illustration purposes only. In the \MP method, the particles are represented through point masses, infinitesimal small points with properties assigned to them.
\FloatBarrier
\begin{figure}[h!]
\centering
\begin{tikzpicture}
...
\end{tikzpicture}
\caption{Grouping particles on the left to parcels on the right with $\wp = 3$. On the right particles are shown as "point masses" as used in the Lagrangian framework. The two sketches on the left are for illustrative purposes to show how grouping works and do not represent the particles used for calculation.}
\label{fig:Parcels}
\end{figure}
\FloatBarrier
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
The interpolation operator for the $x$-coordinate is independent of the $y$ and $z$ coordinates.
...
\begin{tikzpicture}[ultra thick, scale = 1.4]
    ...
    \draw[kit-blue,dotted] (0,0) -- (1,1) -- (2,0) -- (4,0);
    ...
    \draw (2.3,0) circle (1.5pt) node[anchor = north west]{$x_p$};
    %\draw[dotted] (1.5, 0.25) -- (2.5,0.25) -- (2.5, -0.25) -- (1.5, -0.25) -- cycle;
    %\node[anchor = south] at (2, 0.25) {cell $\# i$};
\end{tikzpicture}
\caption{Linear interpolation operator as defined in eq. \eqref{eq:LinIntOp}. On the left $\ip_i$ on the right $\ip_{i-1}$, $\ip_{i}$, $\ip_{i+1}$ with $\ip_{i}$ and  $\ip_{i+1}$ being used for the interpolation of the particle.}
...
In the two-dimensional case, the grid properties are mapped to the particle by using the four enclosing cell centers, see Fig. \ref{fig:finter3DLinear}. 
...
The product of interpolation operators for a grid property $\gp$, where the property is mapped to a particle location and to the grid, is defined as
\begin{equation*}
...
\end{equation*}
that is depicted in Fig. \ref{fig:topHat}.
\begin{figure}[h!]
    \centering
...
For the two-neighbor interpolation, we use the velocity $u$ in $\x$ direction and the velocity $v$ in $y$ direction. As the name suggests, we use the two neighboring velocities, see fig. ~\ref{fig:intTwoNeigh}. We use two top-hat functions for the $\x$-face velocity
\begin{align}
...
\end{equation*}
To simplify the equation, we first look at the product of an interpolation operator with the gradient of an interpolation operator
\begin{equation*}
...
\end{equation*}
We will look more specifically at the interpolation used in \OF later in Sec. \ref{sec:OFInterpolation}.
...
The method can be roughly split into the following three steps in each time step.
...
Then, the momentum and continuity equations are solved on the Eulerian grid.
Derivative terms, for example for the solid stress, are mapped back to the particles and used to calculate the updated particle positions.
This completes the time step.
In the following, we will derive each step and look at the implicit approximations used.

For the numerical solution, we use the parcels mentioned above. As per the assumption made in Eq. \eqref{eq:partConservation}, the number of particles in a volume is conserved in particle phase space. 
Therefore, the parcels have a constant number of particles.
We assume that a particle's mass and volume are constant.
Advanced particle positions are then calculated by
\begin{equation}
    \label{eq:MPposition}
    \xp^{n+1} = \xp^{n} + \Delta t \up^{n+1}, \ n \in \mathbb{N} \qquad \xp^{0} = \x_0
\end{equation}
and the velocity is rearranged by the discretized acceleration \eqref{eq:accel}
...
The equations for updating the position Eq. \eqref{eq:MPposition} and velocity Eq. \eqref{eq:MPvelocity} are used at the end of a time step to calculate the new particle velocities and positions.
These two equations are the first of eight equations, which we will derive in the following.
We start by applying a central differencing scheme to the fluid continuity equation
\begin{equation}
...
A different approach, a finite volume method, is used in \OFns. For now, though, we will stick to the finite differencing scheme used in the original paper.

To approximate the convective flux $\fvf \rf \uf^2$, we split it into the mass flux and the fluid velocity.
...
For the pressure, we use the ideal gas assumption from above \eqref{eq:gasDensReal} that relates fluid pressure and density, which results in
\begin{equation}
...
\end{equation}
Inserting both back into the Taylor expansion Eq.\eqref{eq:pTaylor}
\begin{equation}
...
\end{equation}
and use the cell-face interpolation function
\begin{equation}
...
\end{equation}
The solid stress is highly nonlinear. Using the old velocity and fluid properties when particles are near close pack can push particles into already close-packed spaces.
...
\begin{align}
    ...
    \Rightarrow \tilde{\ss}_i &= \ss_i^n + P_s\dfrac{\pvfcp}{(\pvfcp - \pvf)^2} (\tilde{\pvfi}- \pvfi^n).
\end{align}
To approximate the volume fraction for the provisional parcel positions, we first need to compute it at the cell faces before we can interpolate it to the parcels. 
...
We multiply the discretized momentum Eq. \eqref{eq:discMom} by $\Np \mp$  and sum over all parcels
\begin{equation}
\begin{aligned}
    \sum_p \Np \mp \dfrac{\up^{n+1}-\up^n}{\Delta \tvar} =& - \sum_p \Np \mp \g - \sum_p \dfrac{\Np \mp}{\rp} \left( \dfrac{\tilde{\p}_{i+1}- \tilde{\p}_i}{\Delta \x}\right)_p + \sum_p D_p \Np \mp (\tilde{\up} - \up^{n+1}) \\
...
\end{equation}
Using Eq. \eqref{eq:interpolationPVFCellFace} gives us
\begin{align}
...
For the density and the fluid phase fraction at the provisional parcel position, we avoid the calculation of extra parameters by using known quantities and interpolate to the cell center
\begin{align}
...
Once the grid values are calculated, we can approximate the new particle positions by using the velocity calculated in Eq. \eqref{eq:MPvelocity}.
We then map the particle properties to their positions.
Using the advanced particle velocities and inserting them into the momentum exchange term $\F_{i+\frac{1}{2}}$ and inserting that into the momentum equation, we get the fluid momentum equation.
...
\begin{equation}
    (\x_p^{n+1} - \xpp ) = \Delta \tvar (\up^{n+1} - \up^n) \stackrel{\eqref{eq:errVel}}{=} (\Delta \tvar )^2 ( -\g - \dfrac{1}{\rp} \left( \dfrac{\tilde{\p}_{i+1} - \tilde{\p}_i}{\Delta \x} \right)_p - \dfrac{1}{\rp \pvf} \left( \dfrac{\tilde{\ss}_{i+1} - \tilde{\ss}_i}{\Delta \x} \right)_p + D_p ( \tilde{\up} - \up^{n+1})
\end{equation}
we have an error in $\mathcal{O}(\Delta \tvar )^4$.
Combining the former two equations and using that we can write the derivative of the interpolation as cell face interpolation, see Eq. \eqref{eq:gradInterp}, we have
\begin{equation}
    \begin{aligned}
...
\end{align}
with $\pvf \ \delta \us$ being one of the parameters we solve for later.
We then write the fluid continuity Eq. \eqref{eq:fluidContDiscFull} as
\begin{equation}
...
Because we want to solve for $(\pvf \ \delta \up)$, we want to eliminate the advanced time step $\u_p^{n+1}$ from the equation above. For that, we again use Eq. \eqref{eq:MPvelocity}
\begin{align}
...
\end{align}
Using the same rearrangements as for Eq. \eqref{eq:momentumEquationFluidDisc} we have
\begin{equation}
\label{eq:momentumGasDisc}
\begin{aligned}
    &\rp \dfrac{ (\pvf \ \delta \up )_{i+\frac{1}{2}}}{\Delta \tvar} = \dfrac{1}{\Delta \x (\rf \fvf)_{i+\frac{1}{2}}^n} \sum_p \left[ \Np \mp \ifg_{i+\frac{1}{2}} (\xpp) \dfrac{\d}{1+\Delta \tvar \d} \right] (\fvf \rf \u_g)_{i+\frac{1}{2}}^{n+1} \\
...
We now have everything to assemble the \MP-method, the gas and solid continuity and momentum equations. 
...
The calculation is extensive and does not yield a nice result. We therefore omit it in this master thesis.
...
\color{black}
These two equations are used in tridiagonal solvers for $(\fvf \rf)$ and $\pvf$, where one variable is fixed and the current approximation is used to calculate the other variable alternating until convergence.
Afterwards, $(\pvf \ \delta \up)$ and $(\fvf \rf \uf)$ are solved via equations \eqref{eq:momentumEquationFluidDisc} and \eqref{eq:fluidContDisc}.
...
\centering
    ...
    \tikzset{stretch/.initial=1}
        ...
        \node (pro3) [process, below of = pro2] {Update particle velocities and positions.};
        \node (stop) [startstop, below of = pro3] {End time step loop};
  ...
  (stop.west) --
  ($ ({stop.west}|-{stop.west}) + (-3,0) $) -- % move left from stop.west
  ($ ({stop.west}|-{start.west}) + (-3,0) $) -- % vertical, x fixed from stop.west
  (start.west);
    \end{tikzpicture}
    \caption{Flow chart of the one-dimensional \MP method.}
    \label{fig:FCMPOneDim}
\end{figure}
%\Todo{Include damping terms in diagram}
%\Todo{Nochmal alle wichtigen Gleichungen aufführen?}

So far, we have discussed the numerical solution of the \MP method in one dimension. Since then, the method has been extended to incorporate more advanced particle normal stress models as well as collision terms.
...
From the previous Sec. \ref{sec:damping} we know that the damping particle density can be expressed as
\begin{equation}
    \pd = e^{-\frac{\tvar}{\sd}} \pd^n + \left(1-e^{- \frac{t}{\sd}} \right) \pd_{D},
\end{equation}
see Eq. \eqref{eq:BGKanalytical}.
...
\end{equation}
Here, we have a Dirac delta function which is zero everywhere except at zero. This transforms the distribution into a point mass function, which represents the number of particles at a certain position with a certain velocity and mass. 
The superscript $n$ denotes the discrete time step $\tvar^n$.
% The next time step is then calculated by
% \begin{equation}
%     \tvar^{n+1} = \tvar^n + \Delta t.
% \end{equation}
We start discretizing $\pd_D$ by applying Eq. \eqref{eq:colDamp} to $\pd_d^n$
\begin{align}
...
\end{align}
Substituting the discrete $\pd^n$ and $\pd^n_D$ into the analytical solution Eq. \eqref{eq:BGKanalytical}, we receive the following approximation at time step $t^{n+1}$, with $\Delta \tvar = t^{n+1} - t^{n}$
\begin{equation}
...
A straightforward implementation of this would lead to an exponential increase in parcels, as it would require splitting the parcel into two. One with the initial velocity $\up$ of size $\Np \operatorname{exp}(-\Delta\tvar/ \ss_D)$ and one with the mass-averaged velocity of size $\Np (1-\operatorname{exp}(-\Delta \tvar / \ss_D)$.
...
\end{align}
with $\Psi(\p)$ chosen so that $\pd^{n+1}$ and $\tilde{\pd}$ have the same variance
\begin{equation}
\label{eq:constDamp}
...
Approximating Eq. \eqref{eq:constDamp} with $e^{-x} \approx 1/(1+x)$ and using it to update the particle velocity according to Eq. \eqref{eq:isoAdvance}, we get the new particle velocities
\begin{equation}
...
We use Eq. \eqref{eq:rtiSol} from the previous Sec. \ref{sec:isotropy}
\begin{equation}
...
For the initial discrete distribution $\pd_0$ we use the velocities and positions from the previous time step
\begin{equation}
...
\end{equation}
We can plug this into the solution of the differential Eq. \eqref{eq:rtiSol} and obtain the provisional particle distribution 
\begin{equation}
...
\end{equation}
The advanced particle distribution is obtained by random sampling. The details are explained in the implementation in Sec. \ref{sec:MPLoop}.
In the case that no collision has taken place, the velocity of the particle remains the same as before. Otherwise, we draw from the Gaussian distribution and add the sampled velocity to the current particle velocity.
Since we have a finite number of particles, the new particle velocities will likely not have the same mean velocity and variance as the initial particle distribution. 
To conserve the mean particle velocity and variance cell-wise, we modify the particle velocity $\tilde{\up}$ for particles affected by collision. The new velocity is 
\begin{equation}
\label{eq:rtiUpdate}
