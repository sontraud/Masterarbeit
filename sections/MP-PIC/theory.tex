...
\label{ch:MPPIC}
In this chapter, we discuss the theoretical foundations of the \MP (Multi‑Phase Particle‑In‑Cell) method. We look at the governing equations and how they are solved and at the modeling approaches, especially at the methods used for the particles.
Subsequently, the first numerical implementation of the \MP method is discussed, following the formulation originally presented in \cite{MPPICFirst}.
...
In the \MP method, the fluid phase is described using an Eulerian framework, whereas the particle phase is represented using a Lagrangian approach.
...
\label{eq:FlStTeZero}
    \fst = 0,
\end{equation}
which is also assumed in this chapter \cite{MPPICFirst}, \cite{MP2d},\cite{MPInterpolation}. %The term is again discussed in the \OF implementation as this term only matches the implementation if the condition above \eqref{eq:FlStTeZero} is fulfilled.
In later papers the fluid stress tensor is defined by the Newtonian viscosity law
\begin{equation}
    \label{eq:fst}
...
\end{align}
Here $\uf$ is the mean flow fluid velocity, $\d(\Op,\up, \xp,\tvar)$ is the drag function, discussed in \ref{sec:MPDrag}, $\scs(\xp, \tvar)$ is the solids contact stress, discussed in \ref{eq:solidStress} and $\p$ is the mean flow fluid thermodynamic pressure.
...
Often in the gas momentum equation \eqref{eq:momEul} the pressure gradient is multiplied by the gas volume fraction. This term arises from the second term in \eqref{eq:InterMomTrans}, as shown by the following calculation.
...
The models are summarized in \cite{ErgunWenYu}. The drag terms used in the \OF implementation extend the previous model. They are discussed in \Todo{Referenz Set Up}.
...
This can lead to nonphysical results.
\todo{non-physical oder unphysical?}
...
A modified version of this method is implemented in \OF and explained in section \ref{sec:MPLoop}.
...
\begin{equation*}
    \ss = (\pvf \overline{\rp} + \pvf^2 \overline{\rp} (1 + \gamma) g_0) \Theta
\end{equation*}
...
\end{equation*}
\todo{Lun verwenden wir nie könnte man auch weglassen.}
...
It is defined similar to a BGK-collision term 
\begin{equation}
    \label{eq:colDamp}
...
\end{equation}
The Gaussian velocity distribution is centered around the mean velocity
\begin{equation*}
...
\todo[color=kit-green!40]{Markierung das etwas im Appendix gelandet ist.}
\color{black}
...
\color{ForestGreen}
\todo[color=kit-green!40]{Sehr hässlich... und führt niergends gescheit hin}
The calculation is extensive and does not yield a nice result. We therefor leave it out of this master thesis.
...
\begin{align}
\label{eq:isoAdvance}
    \pd^{n+1} =\sum_p \Np \delta (\x_i - \x_{p,i}^n) \delta (\m - \mp) \delta(\u_i - \u_{p,i}^{n+1}), \qquad \u_{p,i}^{n+1}= (1-\Psi) \overline{\nu}_i
\end{align}
...
\begin{equation*}
    \u_{p,i}^{n+1} = \dfrac{\u_{p,i}^{n} + (\Delta \tvar/ (2 \ss_d)) \overline{\nu}_i}{1+ (\Delta \tvar / (2 \ss_D))}.
\end{equation*}
...
\begin{equation*}
    \overline{\up} = \dfrac{\sum_p \Np \mp \up^n}{\sum_p \Np \mp} \qquad \sigma^2 = \dfrac{\sum_p \Np \mp (\up^n - \overline{\up})^2}{\sum_p \Np \mp} \qquad \overline{\m} = \dfrac{\sum_p \Np \mp}{\sum_p \Np}.
\end{equation*}
